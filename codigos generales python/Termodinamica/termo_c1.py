...
def draw_carnot_cycle(ax, x_label, y_label, title, x_range, y_range,
    ...
    # Ajuste para el líquido-vapor (PV)
        ...
        # Puntos cualitativos para fase líquida y vapor a dos temperaturas,
        # en la región de coexistencia las isotermas se representan
        # como segmentos horizontales (P = constante).
        
        v_liq_c, p_c = 1.5, 2.0  # Punto 1 (Líquido saturado, TC)
        v_vap_c, p_c = 4.5, 2.0  # Punto 2 (Vapor saturado, TC)
        v_vap_h, p_h = 4.0, 4.0  # Punto 3 (Vapor saturado, TH)
        v_liq_h, p_h = 2.0, 4.0  # Punto 4 (Líquido saturado, TH)
        
        # Ajuste de la curva de saturación para que pase por los puntos del ciclo
        ...
        x3, y3 = 4.0, 4.0  # Punto 3 (Vapor saturado, T_H)
        x4, y4 = 2.0, 4.0  # Punto 4 (Líquido saturado, T_H)
        
        # Isoterma C (1 -> 2)
        ax.plot([x1, x2], [y1, y2], 'b-', linewidth=2,
                label='Isoterma C (1->2)')
        
        # Adiabática 1 (2 -> 3) - aproximada como recta
        v_23 = np.linspace(x2, x3, 50)
        p_23 = np.interp(v_23, [x2, x3], [y2, y3])
        ax.plot(v_23, p_23, 'r-', linewidth=2,
                label='Adiabática 1 (2->3)')
        
        # Isoterma H (3 -> 4)
        ax.plot([x3, x4], [y3, y4], 'b-', linewidth=2,
                label='Isoterma H (3->4)')
        
        # Adiabática 2 (4 -> 1) - aproximada como recta
        v_41 = np.linspace(x4, x1, 50)
        p_41 = np.interp(v_41, [x4, x1], [y4, y1])
        ax.plot(v_41, p_41, 'r-', linewidth=2,
                label='Adiabática 2 (4->1)')
        # Adiabática 1 (2 -> 3) - modelada como proceso politrópico (ejemplo: P*V^gamma = cte)
        def adiabatic_liquid_vapor(V, V_start, P_start, V_end, P_end, gamma=1.2):
            cte = P_start * (V_start ** gamma)
            return cte / (V ** gamma)
        v_23 = np.linspace(x2, x3, 50)
        p_23 = adiabatic_liquid_vapor(v_23, x2, y2, x3, y3)
        ax.plot(v_23, p_23, 'r-', linewidth=2,
                label='Adiabática 1 (2->3)')
        ...
        # Etiquetas de los puntos
        ax.text(x1 * 0.9, y1 * 0.9, point_labels[0],
                fontsize=12, ha='right')
        ax.text(x2 * 1.05, y2 * 1.0, point_labels[1],
                fontsize=12, ha='left')
        ax.text(x3 * 1.0, y3 * 1.05, point_labels[2],
                fontsize=12, ha='left')
        ax.text(x4 * 0.9, y4 * 0.9, point_labels[3],
                fontsize=12, ha='right')
        
        # Adiabática 2 (4 -> 1) - modelada como proceso politrópico (ejemplo: P*V^gamma = cte)
        v_41 = np.linspace(x4, x1, 50)
        p_41 = adiabatic_liquid_vapor(v_41, x4, y4, x1, y1)
        ax.plot(v_41, p_41, 'r-', linewidth=2,
                label='Adiabática 2 (4->1)')
        
        # Flechas de dirección (cada arrowprops en varias líneas)
        ax.annotate(
            '',
            xy=(x2, y2),
            xytext=(x1, y1),
